...
# 파일을 있는 형태 그대로 읽음
students <ead.table("C:/r/students.txt", header = T, as.is = T)
str(students)
students
...
students <- read.csv("C:/r/student.csv",fileEncoding = "utf-8") 
students
...
students <- read.csv("C:/r/student.csv", stringsAsFactors = TRUE, fileEncoding = "utf-8") 
str(students)
...
patients <- data.frame(name = c("하나", "두나", "세나", "네나", "다나"), 
                       ...
                       gender=factor(c("M", "F", "M", "K", "F")), # -> 혈액형에 이상값이 존재
...
#연습문제
#01. 지역(region)에 따라 구분하여 총 판매량과 평균 가격   
#      dplyr 라이브러리의 group_by와 summarize 함수 사용
avocado <- read.csv('C:/Users/user/Desktop/수업 내용/R/자료/avocado.csv', header=TRUE, sep=",")
avocado
str(avocado)
library("gapminder")
x_avg <- avocado %>%
...
#02. 지역(region)별, 연도별 총 판매량과 평균 가격  
x_avg <- avocado %>%
  group_by(year) %>%
  summarise(v_avg = mean(Total.Volume), P_avg = mean(AveragePrice))
x_avg
...
#03. 지역(region)별, 연도별, 유기농 여부(type) 총 판매량과 평균 가격  
x_avg <- avocado %>%
...
#연습문제
# 04 1부터 100까지의 수 중에서 3의 배수이면서 4의 배수는 아닌 수의 합을 구하라.
sum <- c(0)  
for(i in 1:100){
  if(i%%3 == 0 && i%%4 != 0){
    sum <- sum + i
...
# 05 x와 n을 입력하면 1부터 n까지의 수 중에서 x의 배수 합을 구해주는 사용자 정의 함수를 만들어라.
times = function(x, n){
  total = 0
  for(i in 1:n){
    if(i%%x == 0){
